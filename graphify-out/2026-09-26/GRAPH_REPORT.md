# Graph Report - MatchSense  (2026-09-26)

## Corpus Check
- 153 files · ~102,701 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1495 nodes · 2188 edges · 113 communities (84 shown, 13 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 57 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `49e7aa3f`
...
- `test_main_end_to_end_pipeline_wiring()` --uses--> `Match`  [INFERRED]
...
Cohesion: 0.16
Nodes (11): PredictionService, Any, Resolve fixture prediction from cache if available, otherwise recompute…, Return sorted list of known teams., Service providing match prediction operations., Return attack and defense parameters for team., Return combined profile with Poisson strengths and XGBoost/Elo stats., Get the model instance for a specific architecture. (+3 more)

### Community 3 - "datetime"
Cohesion: 0.06
Nodes (35): datetime, derive_current_season(), derive_season_from_date(), derive_training_window(), Dynamic Premier League season identification. Single source of truth for…, Return the N-season sliding window ending at current_code. Args: current_code:…, Convert a season start year to the 4-digit code used by football-data.co.uk.…, Convert a season start year to a human-readable label. Example: 2026 → "2026-27" (+27 more)
...
Cohesion: 0.14
Nodes (13): DataFrame, Return normalized feature importances from the booster., Discriminative match outcome predictor using regularized gradient-boosted trees., Index precomputed bounded features for fast lookup during CV., Fit XGBoost classifier on historical match window., XGBoostPredictor, mock_training_data(), fixture (+5 more)
...
Cohesion: 0.09
Nodes (22): 1. Clone & Install Dependencies, 1. Dixon-Coles Bivariate Poisson Process, 2. Environment Setup, 2. Regularized XGBoost Tree Ensemble, 3. Start Database & Run Migrations, 4. Run Data Ingestion & Model Fitting, 5. Launch Application Services, 🧪 Comprehensive Test Suite (+14 more)
...
### Community 47 - "Fixture"
Cohesion: 0.12
Nodes (28): Fixture, ModelArtifact, Serialized model binary and walk-forward verification manifest., Upcoming scheduled Premier League fixture with cached predictions., main(), DataFrame, Session, Score completed matches against pre-match fixture predictions grouped by GW. (+20 more)
...
### Community 56 - "xgboost_model.py"
Cohesion: 0.17
Nodes (12): compute_match_stats_features(), DataFrame, Timestamp, Rolling match statistics and shot quality feature extractor., Compute rolling shot and corner metrics strictly prior to match_date. Resets at…, Feature pipeline orchestrator. Combines form, H2H, temporal, match statistics,…, XGBoost match outcome predictor with window-anchored Elo and cold-start…, fixture (+4 more)
...
Cohesion: 0.08
Nodes (26): Run migrations in 'offline' mode. This configures the context with just a URL…, Run migrations in 'online' mode. In this scenario we need to create an Engine…, run_migrations_offline(), run_migrations_online(), Pydantic settings for MatchSense configuration. Loads from environment…, Application configuration loaded from environment variables., Settings, Base (+18 more)

### Community 63 - "sync_pipeline.py"
Cohesion: 0.10
Nodes (24): FootballDataClient, Football-Data.org API client for upcoming Premier League fixtures., Client for querying Football-Data.org Premier League endpoints., Fetch scheduled fixtures and return normalized fixture dictionaries., default_training_seasons(), download_season_csv(), load_all_seasons(), normalize_football_data_org_name() (+16 more)

### Community 64 - "normalize_team"
Cohesion: 0.18
Nodes (8): normalize_team(), Single source of truth for Premier League team name normalization across…, Normalize any team name variation to MatchSense canonical representation., Any, Return team profile with current Elo, rolling form, and shot metrics. Returns…, Extract honest feature differentials for head-to-head match comparison. Returns…, Return metadata about the fitted XGBoost model., Return model metadata for health endpoint and status reporting.

### Community 65 - "package.json"
Cohesion: 0.11
Nodes (18): name, private, version, class-variance-authority, cmdk, jsdom, lucide-react, openapi-typescript (+10 more)

### Community 66 - "🚀 MatchSense Production Deployment Guide (100% Free Tier)"
Cohesion: 0.22
Nodes (8): 🏗️ Architecture Overview, 💡 Free-Tier Tips & Behaviors, 🔄 How Automated Weekly Syncing Works, 🚀 MatchSense Production Deployment Guide (100% Free Tier), Step 1: Create Free PostgreSQL Database (Neon), Step 2: Configure GitHub Secrets & Run Initial Population, Step 3: Deploy FastAPI Backend on Render (Free), Step 4: Deploy Next.js Frontend on Vercel (Free)
...
### Community 71 - "ModelComparisonBlock.tsx"
Cohesion: 0.26
...
### Community 72 - "api.ts"
Cohesion: 0.15
Nodes (15): dynamic, TeamProfilePage(), HealthBadge(), useHealthStatus(), api, ApiError, ClientHealthStatus, sanitizedBase (+7 more)
...
Cohesion: 0.16
Nodes (15): SimulatorContent(), SimulatorPage(), dynamic, TeamsDirectoryPage(), TeamSelector(), TeamSelectorProps, LeagueStrengthScatter(), TeamScatterPoint (+7 more)

### Community 76 - "index.ts"
Cohesion: 0.16
Nodes (14): dynamic, FixtureCard(), FixtureFilter(), FixtureFilterProps, FixtureGrid(), FixtureGridProps, GameweekHero(), GameweekHeroProps (+6 more)
...
### Community 80 - "TestTwoPassSmallSampleRegularization"
Cohesion: 0.12
...
Cohesion: 0.36
Nodes (8): FixtureFeedStatus, health_check(), HealthResponse, ModelStatus, BaseModel, get, Health check endpoint. Returns model metadata, database connectivity, and…, Return API health status, database connection, fixture feed health, and model…
...
### Community 89 - "test_api_phase3.py"
Cohesion: 0.10
...
## Knowledge Gaps
- **407 isolated node(s):** `nextConfig`, `name`, `version`, `private`, `dev` (+402 more)
...
- **Why does `DixonColesModel` connect `DixonColesModel` to `.predict_score_distribution`, `evaluate.py`, `TestPoissonProperties`, `TestDixonColesModel`, `ndarray`, `test_pipeline.py`, `model_manager.py`, `main.py`, `Fixture`, `BasePredictor`, `run_walk_forward_cv`, `TestTwoPassSmallSampleRegularization`, `_tau`, `sync_pipeline.py`?**
  _High betweenness centrality (0.072) - this node is a cross-community bridge._
- **Why does `XGBoostPredictor` connect `XGBoostPredictor` to `normalize_team`, `test_api.py`, `PredictionService`, `evaluate.py`, `Fixture`, `BasePredictor`, `build_match_features`, `run_walk_forward_cv`, `xgboost_model.py`, `sync_pipeline.py`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `BasePredictor` connect `BasePredictor` to `test_api.py`, `PredictionService`, `DixonColesModel`, `XGBoostPredictor`, `model_manager.py`, `.is_healthy`, `run_walk_forward_cv`, `xgboost_model.py`, `prediction.py`?**
...
- **Are the 9 inferred relationships involving `Fixture` (e.g. with `health_check()` and `list_upcoming_fixtures()`) actually correct?**
  _`Fixture` has 9 INFERRED edges - model-reasoned connections that need verification._